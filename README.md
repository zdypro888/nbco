...
- **老板日报**：同一时刻给超管推全局概览（进行中/过期/待验收/近24小时验收 + 过期任务点名）。原始沟通只进入可检索证据，不会单独触发日报或作为明细主动外发；日报只展开已提炼的摘要、进展、风险、决策和交付物
- **AI 周报**：每周一同一时刻，AI 复用 `company_overview` 的封闭事实给每位超管写叙事周报；封闭自动化不做额外语义检索，但会加载有界的当前用户持久规则
