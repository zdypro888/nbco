...
- 月度人员/知识治理等周期任务首次运行时冻结当期输入快照。调度窗口只负责失败重试，不能改变本期成员集合；新增数据进入下一周期，最终摘要每期只发送一次。
...
- 原始群聊属于可按需查询的 communication 证据；公司概览、老板日报和周报只主动展开结构化摘要、进展、风险、决策与交付物，原始消息本身不会单独触发老板日报。
...
- 封闭后台自动化跳过动态语义检索，但固定加载公司常驻规则与有界的当前用户规则，确保自动运行不绕过已学习的个人要求。
...
- 统一调度数据生命周期任务：跨进程租约、dry-run、运行账本、失败重试和控制中心状态；领域模块只提供基于稳定 ID/终态/保留期的检查与回收函数。
...
- `GET /api/admin/maintenance`：查看维护策略、任务租约和最近运行。
- `POST /api/admin/maintenance/run`：超级管理员 dry-run 或幂等触发实际维护。
...
- 业务事实与审计事实不得注册为自动清理任务；可自动处理的范围只包括可重建派生数据和用途已终结的临时数据。
