...
- 编排器只保留无成功动作证据保护：如果没有任何写入/执行型工具成功，却声称已发送/已创建/已设置/正在执行，会重跑一次要求先调用对应工具；只调用查询工具不能确认完成。
- `action_turns` 是事后事实账本：按用户输入和实际工具调用记录摘要、候选工具、工具成功数、证据 JSON 和 outcome；控制中心日志读取它，而不是只依赖聊天文本。
