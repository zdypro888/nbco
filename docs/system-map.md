...
- 每轮用户输入先由轻量动作规划器判断是否需要改变系统状态、外部投递、派工、授权、部署或执行命令；不要把具体聊天话术写成业务补丁。
- 一旦判定为动作轮次，最终回复必须对应成功工具证据、工具失败证据或明确缺参/无权限/渠道限制；否则编排器自动重跑执行轮并记录拦截。
- `action_turns` 是动作事实账本：记录输入摘要、回复摘要、计划工具、工具成功数、证据 JSON 和 outcome；控制中心日志读取它，而不是只依赖聊天文本。
