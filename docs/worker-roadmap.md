...
- `bind` 写入本机配置，Worker Access Token 与服务端 worker 用户绑定；它不同于真人员工一次性邀请。
...
- 原生 session 兼容性指纹：只有工作目录和 engine runtime fingerprint 都一致才 resume；CLI、参数、模型/provider 配置或相关环境变化会自动轮换原生 session，同时保留中枢 scope、workspace、摘要和历史。
