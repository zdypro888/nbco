...
func workflowTools(d Deps, u *store.User) []ai.Tool {
	return []ai.Tool{
		...
		tool("start_workflow", "启动一个标准工作流。首批支持 material_intake（资料分析入库）与 nbco_upgrade（单 worker 升级 SOP）。需要 AI 员工管理权限；高风险工作流还会执行自身权限校验，且必须在 args.confirm=true 后才会创建任务。",
			obj(map[string]any{
...
func CanStartWorkflow(ctx context.Context, d Deps, u *store.User, name string) (bool, string, error) {
	...
	if strings.TrimSpace(name) == "nbco_upgrade" {
		return false, "nbco_upgrade 只能由超级管理员启动。", nil
	}
	if d.Store == nil {
...
func pickAdminWorkflowWorker(ctx context.Context, d Deps, u *store.User, workerID int64) (*store.User, string, error) {
	...
	ws, err := d.Store.ListAdminWorkers(ctx, ownerID)
	if err != nil {
		return nil, "", err
	}
	if len(ws) == 0 {
		return nil, "没有可用 admin worker。请先创建/绑定 worker，并用 set_worker_admin 标记为 admin worker。", nil
	}
	if d.Workers != nil {
		for _, w := range ws {
